...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function getFileListUrl() {
        return getApiUrl().replace('backend/api/command_handler.php', 'backend/api/file_list.php');
    }
    ...
    async function fileListRequest() {
        try {
            const response = await fetch(getFileListUrl(), {
                method: 'GET',
    ...
    async function loadFiles() {
        ...
        const result = await fileListRequest();
        
        if (result.success) {
            const data = result.data || {};
            currentViewer = data.viewer || null;
            allFiles = Array.isArray(data.files) ? data.files.map(function (file) {
                return {
                    ...file,
            ...
            if (currentUserEmail && currentViewer && currentViewer.email) {
                currentUserEmail.textContent = currentViewer.email;
            }
            updateStats();
    ...
    function updateStats() {
        ...
        const txtFiles = allFiles.filter(f => (f.filename || '').toLowerCase().endsWith('.txt'));
        ...
        if (files.length === 0) {
            filesGrid.innerHTML = `
                <div class="text-center" style="grid-column: 1 / -1; padding: 60px 20px;">
                    ...
                    <p style="color: var(--color-gray-500);">Create or upload your first file to get started</p>
                </div>
            `;
            return;
        }
