...
function resolveAttachmentPath($filename) {
    ...
    $pdo = getDbConnection();
    if ($pdo) {
        $stmt = $pdo->prepare("SELECT filepath FROM files WHERE filename = :filename LIMIT 1");
    ...
    $candidate = rtrim($uploadsRealPath ?: UPLOADS_PATH, DIRECTORY_SEPARATOR) . DIRECTORY_SEPARATOR . $normalized;
